...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОстаткиНоменклатуры.Номенклатура КАК НоменклатураРН,
	|	ОстаткиНоменклатуры.Количество КАК КоличествоРН,
	|	ОстаткиНоменклатуры.Стоимость КАК СтоимостьРН
	|ПОМЕСТИТЬ втРасходнаяНакладная
	|ИЗ
	...
	|		ГДЕ
	|			РасходнаяНакладная.Основание = &РТУ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втВозврат.Номенклатура,
	|	втВозврат.Номенклатура.Представление КАК НоменклатураПредставление,
	|	втВозврат.Количество,
	|	ЕстьNULL(втРасходнаяНакладная.КоличествоРН, 0) КАК КоличествоРН,
	|	ЕстьNULL(втРасходнаяНакладная.СтоимостьРН, 0) КАК СтоимостьРН
	|ИЗ
	|	втВозврат КАК втВозврат
	|		ЛЕВОЕ СОЕДИНЕНИЕ втРасходнаяНакладная КАК втРасходнаяНакладная
	|		ПО втВозврат.Номенклатура = втРасходнаяНакладная.НоменклатураРН";

	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Запрос.УстановитьПараметр("РТУ", Основание);
	Выборка = Запрос.Выполнить().Выбрать();

	Пока Выборка.Следующий() Цикл
		Если Выборка.Количество > Выборка.КоличествоРН Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru='В реализации недостаточно товара %1 в количестве %2'"), 
													Выборка.НоменклатураПредставление, 
													Выборка.Количество - Выборка.КоличествоРН
											) ;
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , , , Отказ);
		КонецЕсли;
		Если Отказ Тогда
		...
		Движение.Стоимость = Выборка.СтоимостьРН/Выборка.КоличествоРН * Выборка.Количество;
